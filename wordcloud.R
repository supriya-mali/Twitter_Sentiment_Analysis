modiinuae.tweets = searchTwitter("modiinuae", lang="en", n=1500, resultType="recent")
modiinuae_text = sapply(modiinuae.tweets, function(x) x$getText())
df <- do.call("rbind", lapply(modiinuae.tweets, as.data.frame))
modiinuae_text <- sapply(df$text,function(row) iconv(row, "latin1", "ASCII", sub=""))
str(modiinuae_text)

#corpus is a collection of text documents
library(tm)
quake_corpus <- Corpus(VectorSource(modiinuae_text))
quake_corpus
...
quake_clean <- tm_map(quake_clean, removeWords, c("modiinuae"))#removing search words
#wordcloud
